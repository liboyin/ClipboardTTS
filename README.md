...
Because XCTest launches the hosted macOS app, macOS can request Keychain access approval before the test run proceeds. Approve that system prompt before treating a stalled `check-coverage.sh` run as a test failure; tests use fake or in-memory credentials, so never enter, replace, or share an API key for the prompt.

The unit-test bundle is hosted inside the app, so `UserDefaults.standard` in a test is the real app's defaults domain. `MockURLProtocolTestCase` automatically isolates every mock-network test, clears every declared settings key, and restores the developer's exact configuration only after its sessions, protocol loads, manager initialization, and queued mock work are quiescent. XCTest runs teardown blocks before `tearDown()`, so this lifecycle owns its snapshot directly; do not move restoration to `addTeardownBlock`. Non-network tests that touch settings must call `isolateAppSettingsDefaults()` themselves (`Tests/UserDefaultsSnapshot.swift`).

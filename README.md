...
Because XCTest launches the hosted macOS app, its `ClipboardTTSApp.init` currently runs before XCTest `setUp()` and constructs the production secret store. A Keychain read can therefore block the hosted suite before test-level fakes or settings isolation exist; do not enter, replace, or share an API key if macOS presents a prompt. Task 26 in `TODO.md` owns isolation of this startup boundary, so treat a stalled `check-coverage.sh` run there as a known blocking defect rather than assuming an individual test's fake credentials protect it.

The unit-test bundle is hosted inside the app, so `UserDefaults.standard` in a test is the real app's defaults domain. After `MockURLProtocolTestCase.setUp()` begins, it isolates every mock-network test, clears every declared settings key, and restores the developer's exact configuration only after its sessions, protocol loads, and manager initialization are quiescent. XCTest runs teardown blocks before `tearDown()`, so this lifecycle owns its snapshot directly; do not move restoration to `addTeardownBlock`. It does not yet own or drain factory-created audio-delivery queues before restoration; Task 27 in `TODO.md` owns that lifecycle gap. Non-network tests that touch settings must call `isolateAppSettingsDefaults()` themselves (`Tests/UserDefaultsSnapshot.swift`).
